 ...
 * 삼성전자 공식 워드마크 (SVG 패스).
 *
 * **글꼴로는 재현할 수 없다.** 삼성 워드마크는 전용 레터폼이라 `font-black` 으로 친
 * "SAMSUNG" 과 글자 모양이 다르다 — 특히 A 의 가로획이 없고 U 가 아래로 열려 있다.
 * 그래서 공식 로고의 패스를 그대로 인라인한다(출처: Wikimedia Commons, Samsung wordmark.svg).
 *
 * 색은 `currentColor` 라 쓰는 자리에서 정한다 — 남색 바 위에서는 흰색,
 * 흰 바탕에서는 삼성 블루(#1428A0, 원본 색과 같다).
 * 세로 비율은 viewBox 가 잡으므로 `height` 만 주면 가로는 따라온다.
 */
export default function SamsungWordmark({
  className = '',
  height = 18,
  ...
  return (
    ...
    >
      <defs>
        {/* id 는 페이지 안에서 유일해야 한다 — 흔한 이름을 쓰면 다른 SVG 와 충돌한다 */}
        <clipPath id="samsung-wordmark-clip" clipPathUnits="userSpaceOnUse">
          <path d="M 0,166.885 H 628.238 V 0 H 0 Z" />
        </clipPath>
      </defs>
      <g transform="matrix(12.944053,0,0,-12.944053,-540.03625,1620.0233)">
        <g clipPath="url(#samsung-wordmark-clip)">
