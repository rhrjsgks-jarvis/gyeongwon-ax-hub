...
/**
 * 「전체 재수집」만 취소한다 — **한 번 켜지면 끝날 때까지 매일 쿼터를 다 먹는다.**
 *
 * 2026-09-04 실측으로 드러난 구멍이다. `_forceFull` 은 **한 바퀴를 완전히 마쳐야만**
 * 내려가는데(`!stopped && isFull && !kindDown`), 한 바퀴가 20,858회라 하루 한도에
 * 육박한다. 그래서 못 끝내면 다음 날 또 처음부터 전체 재수집이고, 그 사이 새 글을
 * 잡는 「최근 것만」은 한 번도 못 돈다. 실제로 이틀째 그 상태였다
 * (`fullAt 2026-09-02` 인데 `forceFull true` · `cursor 44/65` · `added 0`).
 *
 * **그런데 그 상태를 벗어날 길이 화면에 없었다.** 있는 것은 「이어달리기 멈춤」
 * (트리거만 끄고 표식은 그대로)과 되돌릴 수 없는 「자료 비우고 처음부터」뿐이었다.
 *
 * **되돌릴 수 있는 조작이라 인증을 걸지 않는다**(`stopSweep` 과 같은 급) — 다시 누르면
 * 그만이고, 잘못 눌러도 잃는 것은 「전부 훑기」한 번뿐이다.
 *
 * **커서는 건드리지 않는다.** 지우면 이미 훑은 44곳을 다시 훑어 쿼터를 또 태운다 —
 * 이 함수를 만든 이유와 정반대가 된다. 남은 매장은 「최근 것만」으로 이어 돌면 된다.
 */
function cancelFull() {
  var was = String(props_().getProperty('_forceFull') || '') === '1';
  if (!was) return '전체 재수집 중이 아닙니다.';
  props_().deleteProperty('_forceFull');
  sumCacheClear_();          /* 화면이 최대 6시간 옛 값을 들고 있지 않게 */
  var cur = Number(props_().getProperty('_cursor') || 0);
  return '전체 재수집을 껐습니다 — 다음 회차부터 새 글만 훑습니다'
    + (cur ? '(' + cur + '번째 매장부터 이어 돕니다).' : '.');
}
...
/* **붙여넣은 판을 밖에서 확인하는 표식**(2026-09-07). 서버 코드는 응답 스키마를
...
function freshState_(d, full) {
  try {
    ...
    if (full) {
      ...
      /* **어느 판이 돌고 있는가** — 붙여넣기 확인이 이 한 줄로 끝난다.
         **캐시에 넣지 않는다**(여기는 `freshState_` 다) — 굳으면 확인이 거짓이 된다. */
      d.gsVer = GS_VER;
