...
(function (global) {
  ...
  var S = {
    ...
    /* 링 — 띠 하나. 원 둘 사이 간격이 곧 반지 두께라 3px 아래로 내리면 16px 에서 붙는다 */
    ring:        '<circle cx="12" cy="12" r="7.2"/><circle cx="12" cy="12" r="4"/>',
    /* 핏 — 밴드형. 워치(네모 판)와 갈리게 몸통을 세로로 길쭉한 알약으로 그린다 */
    band:        '<rect x="8.7" y="5.4" width="6.6" height="13.2" rx="3.3"/><path d="M10.3 5.4V3.1h3.4v2.3M10.3 18.6v2.3h3.4v-2.3"/>',
    /* XR — 고글. **렌즈를 원 둘로 그리면 사운드바와 구분이 안 된다**(둘 다 '가로 상자 +
       원 둘' 이라 22px 에서 같은 그림으로 보였다). 머리 밴드 호와 코 자리 홈으로 가른다. */
    xr:          '<path d="M2.6 10.4a9.4 9.4 0 0 1 18.8 0"/><path d="M2.5 10.4h19v3.6a3 3 0 0 1-3 3h-2.6a2 2 0 0 1-1.7-1l-.8-1.3a2 2 0 0 0-3.4 0l-.8 1.3a2 2 0 0 1-1.7 1H5.5a3 3 0 0 1-3-3v-3.6Z"/>',
    /* SSD·메모리 — 메모리카드. 잘린 모서리가 45° 라 저장장치로 읽힌다(사각형만이면 냉동고와 닮는다) */
    storage:     '<path d="M6.5 3.5h7.4L18 7.6V19a1.5 1.5 0 0 1-1.5 1.5h-10A1.5 1.5 0 0 1 5 19V5a1.5 1.5 0 0 1 1.5-1.5Z"/><path d="M8.8 6.3v3.5M11.6 6.3v3.5M14.4 8.1v1.7"/>',
    /* 선풍기 — 그릴·살·목·받침. 살이 없으면 링과 구분이 안 된다 */
    fan:         '<circle cx="12" cy="9" r="6.5"/><circle cx="12" cy="9" r="1.7"/><path d="M12 2.5v4.8M12 15.5v-4.8M5.5 9h4.8M18.5 9h-4.8"/><path d="M12 15.5v4.2M8.8 20.5h6.4"/>',
    ...
    /* 지도 핀 — 압정(pin)과 다른 그림이다. 압정은 "여기 붙여 둠", 이쪽은 "여기 있음(위치)" */
    place:       '<path d="M12 21.3c3.7-4.5 5.6-7.8 5.6-10.3a5.6 5.6 0 1 0-11.2 0c0 2.5 1.9 5.8 5.6 10.3Z"/><circle cx="12" cy="10.7" r="2.2"/>',
  ...
  var RULES = [
    ...
    [/태블릿|탭\b/, 'tablet'],
    [/워치|시계/, 'watch'],
    /* **앵커 없이 쓰지 말 것** — 맨 `/링/` 은 '업스케일링'·'리마스터링'을, 맨 `/핏/` 은
       '1도어 키친핏'을 문다(제품 상세검색이 이미 데인 부분일치 함정이다). 그래서 브랜드
       이름을 함께 요구하거나 낱말 경계를 붙인다. 정확한 라벨은 위 KEYS 가 이미 받는다. */
    [/갤럭시\s*링|스마트\s*링/, 'ring'],
    [/갤럭시\s*핏|갤럭시\s*fit/i, 'band'],
    [/\bXR\b|확장\s*현실|스마트\s*글래스/i, 'xr'],
    [/\bSSD\b|외장\s*하드|메모리\s*카드|마이크로\s*SD/i, 'storage'],
    [/선풍기|서큘레이터/, 'fan'],
  ...
  var KEYS = {
    ...
    /*
     * **AS 앱 품목 중 아래 RULES 로는 잡을 수 없는 다섯.** 전부 부분일치가 위험한 말이라
     * 정확 일치로만 받는다 — `/링/` 은 '업스케일링'·'리마스터링'에, `/핏/` 은
     * '1도어 키친핏'에 걸린다(제품 상세검색이 이미 데인 함정이다).
     */
    '링': 'ring', '핏': 'band', 'XR': 'xr', 'SSD·메모리': 'storage', '선풍기': 'fan',
  ...
  var EMOJI = {
    ...
    '📌': 'pin', '📍': 'place', '📤': 'share', '➕': 'plus', '▶': 'play',
    '↔': 'arrows', '↕': 'arrows', '↗': 'arrow_ne',
    '🔥': 'fire', '♨': 'fire', '💨': 'wind', '🌀': 'wind', '🌬': 'wind', '💧': 'drop',
    '📸': 'camera', '💬': 'chat', '💰': 'money', '🏬': 'store',
    '📖': 'book', '📚': 'book', '🎓': 'edu', '👆': 'tap',
    /* 설치비용·사전준비에서 쓰던 이모지 (2026-08-31 사장님 지시로 심벌 전환).
       **이 저장소는 이모지를 쓰지 않는다** — OS 마다 모양이 달라지고 삼성 블루와
       무관한 색이 튀며 키우면 뭉갠다. 그 앱만 규칙 밖에 있었다. */
    '🧾': 'doc', '🔌': 'bolt', '🗄': 'store', '📦': 'truck',
