...
const MODULE_GROUPS: { title: string; modules: ModuleCard[] }[] = [
  ...
  {
    /*
     * 자유게시판은 매장운영 도구에 넣지 않는다 — 2026-08-31 사장님 지시:
     * *"매장운영도구말고 별도로 빼주세요 위치는 매장운영도구 하단"*. 이름도
     * '팀 게시판'에서 바꿨다(같은 지시).
     */
    title: '자유게시판',
    modules: [
      {
        ...
        icon: 'chat',
        title: '자유게시판',
        desc: '공지·정보·질문·건의 — 경원영업팀이 함께 적는 자리',
        color: '#1428A0',
        bg: '#EEF2FF',
        updated: '2026.08',
        status: 'live',
...
/*
 * 매장 바이럴 — 우리 매장이 언급된 블로그·카페 후기 (2026-08-31 사장님 요청).
 *
 * **앱 안 화면이 아니라 구글 스크립트로 나간다.** 시크릿쿠폰·컨시어지와 같은 형태다.
 * 그렇게 만든 이유는 사장님 지적 — *"vercel 앱은 전산PC에서 보안이슈로열리지않아
 * 구글스크립트와같은 경로로"*. 수집·저장·화면이 전부 구글 안에서 끝난다.
 *
 * **주소는 비밀값이 아니라 공개 링크라 커밋해도 된다**(시크릿쿠폰이 이미 그렇다).
 * 네이버 API 키는 그쪽 스크립트 속성에만 있고 이 저장소에는 없다.
 *
 * 실측(2026-08-31 첫 수집) — 65개 매장 × 블로그·카페 130 회 호출 · 받음 2,358 →
 * **저장 1,458건** · 후기가 잡힌 매장 49곳 · 카페 251곳(다이렉트웨딩 319건이 1위).
 */
const VIRAL_LINKS = [
  {
    href: 'https://script.google.com/macros/s/AKfycbyfiNnGIrydVPOOs5BlcsMCgKFtv2EWfWQFjEqU1lZGNFHYoonRW2CTkwOi5-aPG4Q/exec',
    icon: 'chat' as IconName,
    label: '매장 바이럴 대시보드',
    desc: '일간·주간·월간·누적 · 매장을 누르면 그 후기 링크만',
  },
]
