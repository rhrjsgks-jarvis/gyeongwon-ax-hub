...
const MODULES = [
  { href: '/finder',  icon: '🔍', title: '모델파인더',   desc: '키워드 한 줄로 CE·MX 전 제품(206종) 검색',                     color: '#1428A0', bg: '#EEF2FF', updated: '2026.06', status: 'live' },
  { href: '/care',    icon: '💚', title: 'AI Care 검색기', desc: '구독케어 서비스 항목·조건 즉시 조회',                            color: '#059669', bg: '#ECFDF5', updated: '2026.06', status: 'live' },
  { href: '/test',    icon: '📝', title: '레벨업테스트',  desc: '2026 제품 전문가 역량 평가 · 25문항 · 30분',                    color: '#7C3AED', bg: '#F5F3FF', updated: '2026.06', status: 'live' },
  { href: '/compare', icon: '⚡', title: 'AI 생성기',    desc: '타사비교 가이드 · URL 퀴즈 — URL 입력으로 즉시 생성',           color: '#EA580C', bg: '#FFF7ED', updated: '2026.07', status: 'live' },
]
...
function StatusBadge({ status }: { status: string }) {
  return status === 'live'
    ? <span className="badge text-emerald-700 bg-emerald-50 border border-emerald-200">✓ 운영중</span>
    : <span className="badge text-amber-700 bg-amber-50 border border-amber-200">🔧 구축중</span>
}
...
export default function Home() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="rounded-2xl p-4 mb-6 text-white text-sm" style={{ background: 'linear-gradient(135deg, #1428A0, #2563EB)' }}>
        <div className="flex items-start gap-3">
          <span className="text-xl">🚀</span>
          <div>
            <p className="font-bold mb-0.5">AX 허브 구축 30일 플랜 · Day {clampedDay}</p>
            <p className="opacity-80 text-xs">4개 AI 영업지원 도구 운영 중 · 매일 업데이트 예정</p>
          </div>
        </div>
      </div>
          ...
          <Link key={mod.href} href={mod.href} className="no-underline">
            <div className="module-card group">
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: mod.bg }}>{mod.icon}</div>
                <StatusBadge status={mod.status} />
              </div>
              <h2 className="font-bold text-base mb-1 group-hover:text-[#1428A0] transition-colors">{mod.title}</h2>
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="flex-1 bg-gray-100 rounded-full h-2">
            <div className="h-2 rounded-full transition-all" style={{ width: `${progress}%`, background: 'var(--color-primary)' }} />
          </div>
          <span className="text-xs font-semibold text-gray-500">Day {clampedDay} / {TOTAL_DAYS}</span>
        </div>
        <div className="grid grid-cols-4 gap-2 text-center mt-3">
          {weeks.map((w) => (
            <div key={w.label} className="rounded-xl py-2 px-1" style={{ background: w.active ? 'rgba(20,40,160,0.08)' : '#F9FAFB', border: w.active ? '1.5px solid rgba(20,40,160,0.2)' : '1.5px solid transparent' }}>
              <p className="text-xs font-bold" style={{ color: w.active ? 'var(--color-primary)' : '#9CA3AF' }}>{w.label}</p>
