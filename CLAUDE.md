...
회귀 테스트 스크립트가 `scripts/test-install.mjs`에 저장되어 있다 (매 세션 재작성할 필요 없음). 변경 후 항상:

```bash
node scripts/test-install.mjs   # 20개 카테고리 전체 렌더링, 이미지 개수/카드노출/링크 유효성, 키워드 검색 검증
npx tsc --noEmit                # 타입체크
```

새 카테고리를 추가하거나 이미지 개수가 바뀌면 `scripts/test-install.mjs`의 `expectedImageCounts`를 반드시 함께 갱신할 것 — 안 하면 테스트가 실패한다.

커밋 전 실수로 생성되는 `tsconfig.tsbuildinfo`, `package-lock.json`은 git에 올리지 말 것 (과거 여러 번 실수로 커밋되었다가 별도 정리 커밋이 필요했음).
