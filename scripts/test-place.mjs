 ...
 */
{
  ...
  /* 아직 도면도 안 올렸는데 "크기를 맞춰 주세요"는 이르다 — 할 차례가 아닌 일을 시키면 헤맨다 */
  ok2(has("'도면을 가져오면 시작합니다'"),
    '[18] 시작 화면에서는 "크기를 맞춰 주세요"라 하지 않는다');
}

/*
 * [19] **축척 검산** (2026-08-30). 상담사는 *"이 벽이 몇 mm 입니까"* 한 번으로 집 전체
 * 크기를 정한다. 그 답이 두 배 틀리면 **넓이는 네 배**가 되고 그 위에서 내린
 * "들어갑니다 / 안 들어갑니다"가 전부 거짓이 되는데 **화면에 아무 표시도 안 났다.**
 * (이 밤 작업 중 실제로 겪었다 — 3600mm 를 넣었더니 침실이 4.1㎡ 로 잡혔다.)
 *
 * 브라우저 실측 — 대방 엘리움 86A(전용 85.53㎡):
 *   축척 ×1   합계 105㎡ → 조용함        ✔
 *   축척 ×0.5 합계  30㎡ → 경고          ✔
 *   축척 ×2   합계 330㎡ → 경고          ✔
 */
{
  const has = (t) => html.includes(t);
  ok2(has('function scaleSanity()'), '[19] 축척 검산 함수가 있다');
  ok2(has('if (ratio >= 0.45 && ratio <= 2.2) return null;'),
    '[19] 범위를 넓게 잡는다 — 좁히면 멀쩡한 도면이 매번 경고를 달고 아무도 안 읽는다');
  ok2(has('state.rooms.length < 3'),
    '[19] 공간이 셋도 안 잡혔으면 판정하지 않는다 (합계가 뜻이 없다)');
 ...
 * 떨어진다. 화면에는 *"냉장고 · 거실"* 이라 적히고 상담사는 *"왜 여기 있지"* 한다.
 *
 * 실측(공용 벽선 12건 전수 자동 배치) — **감일·하남 6건에서 냉장고·김치냉장고·세탁기가
 * 전부 거실로 갔다.** 그 벽선들에 주방·세탁실이라는 이름의 방이 없어서다(LH 도면).
 * 안내를 붙인 뒤 다시 돌리니 **필요한 7건에만 뜨고** 제대로 잡힌 5건에는 안 뜬다.
 *
 * **자동으로 고치지 않는다** — 어디가 주방인지 모르는 것은 사실이고, 지어내면 그 위에서
 * "여기 놓으면 됩니다"가 거짓이 된다.
 */
{
  const has = (t) => html.includes(t);
  ok2(has('function roomsMissing()'), '[20] 갈 방이 없는 것을 찾는 함수가 있다');
  ok2(has("if (use === '거실' || use === '침실') continue;"),
    '[20] 거실·침실은 세지 않는다 — 폴백이 거실이라 뜻이 없다');
  ok2(has('이(가) 이 도면에는 안 잡혔습니다'), '[20] 화면이 그 사실을 적는다');
  ok2(has('지금은 가까운 벽에 두었으니 끌어서 옮겨 주세요'),
    '[20] 무엇을 하면 되는지까지 말한다');
  /* 놓은 가전이 없으면 말할 것도 없다 — 빈 화면에 경고를 띄우면 고장으로 읽힌다 */
  ok2(has('if (!state.rooms.length || !state.items.length) return [];'),
