...
(async () => {
   ...
   */
  {
    ...
    if (!T) fail('[15] ELEC_TARIFF 를 앱에서 못 읽음');
      ...
      /*
       * 기후환경요금은 2026-08-16 에 채웠다 — RPS 7.7 + ETS 1.1 + 재정지원 0.2 = **9.0원/kWh**
       * (한전 사이버지점 원문. 웹검색 요약은 5.3 이라 했는데 원문과 달랐다).
       * 부가가치세 10%·전력산업기반기금 **2.7%** 도 함께 넣었다(한전 한글 전기요금표 원문 —
       * 흔히 3.7% 로 알려져 있어 그대로 넣을 뻔했다).
       * 값이 또 바뀌면 여기가 먼저 실패한다 — 출처 주석과 골든값을 함께 고칠 것.
       */
      if (T.climate !== 9.0) fail(`[15] 기후환경요금 단가가 ${T.climate} 다 — 확인한 값은 9.0(RPS 7.7+ETS 1.1+재정지원 0.2)`);
      if (T.fund !== 0.027) fail(`[15] 전력산업기반기금이 ${T.fund} 다 — 한전 원문은 2.7%(3.7% 가 아니다)`);

      /*
       * 골든값은 **손으로 검산한 값**이다(2026-08-16, 기타계절):
       *   200kWh → 기본 910 + 전력량 200×120.0 + (연료비 5.0+기후환경 9.0)×200
       *          = 27,710 → 부가세 2,771 + 기반기금 740 = 31,221
       *   350kWh → 1,600 + (200×120.0 + 150×214.6) + 14×350
       *          = 62,690 → 부가세 6,269 + 기반기금 1,690 = 70,649
       */
      const cases = [[200, 31221], [350, 70649]];
   ...
   */
  {
    /* 소음 값을 가져도 되는 모델 = 원문에 값과 시험 조건이 함께 적힌 것 */
    const NOISE_OK = {
      ...
      /*
       * 로봇청소기 — 2026-08-16. 지원 페이지 원문이 4종 모두 같은 값을 준다:
       * "소음 = (물걸레+먼지흡입 마루) 저소음 63dB / (자동먼지비움) 87dB".
       * 예전에는 55 로 적어 LG 58 보다 조용하다고 등급까지 붙였다 — 우리 제품에
       * 유리한 방향으로 틀린 값이었다. 타사는 전부 출처가 없어 지웠다(LG 는 사양표에
       * 항목 자체가 없고, 로보락·에코백스·드리미는 지면에 dB 표기가 0건이다).
       */
      'VR90F01SAG': 63,
      'VR80F01ADH': 63,
      'VR80F01ADG': 63,
    ...
    {
      /*
       * 아직 **원문 각주까지 확정하지 못한** 카테고리의 소음 칸 — 실패로 세지 않고 목록만 남긴다.
       *
       * 2026-08-30 재고: 냉장고 7 · 에어컨 9 · 김치냉장고 2칸. 삼성 지원 페이지와
       * **삼성닷컴 전수 수집분(1,896종)에 그 수치가 한 건도 없다** — 남은 출처는
       * dp 카탈로그뿐이다. 값을 지우지 않았다(틀렸다는 근거가 아니라 못 찾은 것이다).
       * 다음 카탈로그 대조 때 에어컨 5종부터 볼 것 — 삼성닷컴을 다시 두드리지 말 것.
       */
      const todo = [];
      for (const [cat, c] of Object.entries(DB)) {
