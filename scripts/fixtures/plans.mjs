...
export const PLANS = [
  ...
  {
    // 닫기 반경 사다리의 **눈금 사이에서만** 닫히는 작은 방.
    //
    // 실제 LH 84㎡ 평면도에서 욕실(폭 1.4m)과 드레스룸이 "경계를 닫지 못했습니다"로
    // 인식 자체가 실패했다. 원인은 알고리즘이 아니라 탐색이었다 — 욕실은 361~417mm,
    // 드레스룸은 612~695mm 에서만 깔끔히 닫히는데 사다리가 300·500·800 이라 그 구간을
    // 통째로 건너뛰었다. 작은 방일수록 "새지 않으면서 방이 살아남는" 창이 좁아 눈금 사이에 빠진다.
    //
    // 여기서는 그 상황을 재현한다: 남쪽 벽이 4m 트여 있어 작은 반경에서는 도면 밖으로 새고,
    // 욕실 개구부가 1,100mm 라 막으려면 550mm 이상이 필요한데, 방 반폭이 800mm 라
    // 800mm 반경에서는 방이 통째로 메워진다 → 사다리에 없는 550~800mm 에서만 성립한다.
    name: '작은 욕실 (사다리 눈금 사이에서만 닫히는 크기)',
    svg: planSvg({
      w: 9000, h: 6500, ppm: 0.16,
      body(a) {
        const M = 300;
        const W = 8400, H = 5900;
        a.wall(M, M, W, T_BEAR);                       // 북
        a.wall(M, M + H - T_BEAR, W, T_BEAR);          // 남
        a.wall(M, M, T_BEAR, H);                       // 서
        a.wall(M + W - T_BEAR, M, T_BEAR, H);          // 동
        // 남쪽 6,000 트임(발코니 확장) — 작은 반경에서는 여기로 새어 나간다.
        // 누수 판정이 "도면 가장자리 접촉 길이 > (가로+세로)×0.3" 이라, 트임이 좁으면
        // 새고도 누수로 잡히지 않아 세대 전체가 한 방으로 통과해 버린다.
        a.cut(M + 1200, M + H - T_BEAR, 6000, T_BEAR);
        // 욕실 1,600 × 2,100 (내부) — 동쪽 벽에 1,100 개구부
        const bx = M + T_BEAR, by = M + T_BEAR;
        a.wall(bx + 1600, by, 150, 2100 + 150);
        a.wall(bx, by + 2100, 1600 + 150, 150);
        a.cut(bx + 1600, by + 700, 150, 1100);         // 개구부 1,100
        a.thin(`M${a.px(bx + 1600)} ${a.px(by + 700)} a ${a.px(1100)} ${a.px(1100)} 0 0 1 ${a.px(1100)} ${a.px(1100)}`);
        a.text(bx + 300, by + 1200, '욕실');
        a.text(M + 4200, M + 3600, '거실');
      },
    }),
    mmPerImgPx: 1 / 0.16,
    probes: [
      { at: [1300, 1500], areaM2: [2.6, 4.2],
        note: '욕실 1,600×2,100 = 3.36㎡. 550~800mm 에서만 닫히므로 사다리가 성기면 인식 자체가 실패한다' },
    ],
  },
  {
    // 실제 분양 도면은 "한 장 = 한 평면도"가 아니다. LH 단지 4곳의 2D 평면도 34장을
    // 전수로 돌려 보고 알아낸 관례를 아래 셋으로 옮겼다.
    //
    // ① 마감재 안내도 — 도면 옆에 범례 블록이 붙는다(이미지의 25%를 차지하기도 한다).
    //    범례를 건물로 오인하면 도면 범위가 부풀어 누수 판정이 무너진다.
    name: '마감재 안내도 (도면 옆에 범례 블록)',
    svg: planSvg({
      w: 11400, h: 5600, ppm: 0.11,
      body(a) {
        const M = 300, W = 7000, H = 5000;
        a.wall(M, M, W, T_BEAR); a.wall(M, M + H - T_BEAR, W, T_BEAR);
        a.wall(M, M, T_BEAR, H); a.wall(M + W - T_BEAR, M, T_BEAR, H);
        a.wall(M + 3400, M + T_BEAR, 150, H - T_BEAR * 2);       // 가운데 칸막이
        a.cut(M + 3400, M + 1800, 150, DOOR);                    // 문 900
        a.thin(`M${a.px(M + 3550)} ${a.px(M + 1800)} a ${a.px(DOOR)} ${a.px(DOOR)} 0 0 1 ${a.px(DOOR)} ${a.px(DOOR)}`);
        a.text(M + 1200, M + 2600, '침실1'); a.text(M + 4600, M + 2600, '거실');
        // 범례 블록 — 건물 밖 오른쪽. 테두리 + 색 견본 + 설명줄
        a.thin(`M${a.px(7800)} ${a.px(300)} h${a.px(3000)} v${a.px(2200)} h${a.px(-3000)} z`, 2);
        for (let i = 0; i < 5; i++) {
          a.fill(8000, 600 + i * 380, 500, 160, ['#2ea7e0', '#e5007f', '#f29500', '#8fc31f', '#8e6bbf'][i]);
          a.thin(`M${a.px(8700)} ${a.px(700 + i * 380)} H${a.px(10600)}`, 3);
        }
      },
    }),
    mmPerImgPx: 1 / 0.11,
    probes: [
      { at: [2000, 2800], areaM2: [12, 18], note: '침실1 3,300×4,600 = 15.2㎡ — 범례를 건물로 오인하면 여기서 새어 나간다' },
      { at: [5400, 2800], areaM2: [11, 17], note: '거실 3,150×4,600 = 14.5㎡. 둘이 합쳐지면 29.7㎡가 되어 걸린다' },
    ],
  },
  {
    // ② 옵션 안내도 — 본 도면 옆에 "공간옵션" 별도 도면이 함께 실린다.
    //    한 장에 건물이 둘이라 도면 범위가 둘을 함께 감싼다.
    name: '옵션 안내도 (한 장에 도면 둘)',
    svg: planSvg({
      w: 12600, h: 4600, ppm: 0.12,
      body(a) {
        const H = 4000;
        const unit = (x0) => {
          a.wall(x0, 300, 5000, T_BEAR); a.wall(x0, 300 + H - T_BEAR, 5000, T_BEAR);
          a.wall(x0, 300, T_BEAR, H); a.wall(x0 + 5000 - T_BEAR, 300, T_BEAR, H);
          a.thin(`M${a.px(x0 + 1500)} ${a.px(370)} H${a.px(x0 + 3500)} M${a.px(x0 + 1500)} ${a.px(430)} H${a.px(x0 + 3500)}`);
        };
        unit(300);            // 본 도면
        unit(7300);           // 공간옵션 (별도 도면)
        a.text(1800, 2400, '침실2'); a.text(8800, 2400, '침실2');
        a.thin(`M${a.px(7300)} ${a.px(200)} h${a.px(5000)} v${a.px(4200)} h${a.px(-5000)} z`, 2);
      },
    }),
    mmPerImgPx: 1 / 0.12,
    probes: [
      { at: [2800, 2300], areaM2: [14, 19], note: '본 도면 4,600×3,600 = 16.6㎡' },
      { at: [9800, 2300], areaM2: [14, 19], note: '옵션 도면도 같은 크기로 잡혀야 한다 — 둘을 한 건물로 보면 그 사이 빈 곳이 방이 된다' },
    ],
  },
  {
    // ③ 마감재 표시 — 방 안에 컬러 배지(숫자 원형)가 흩어져 있고, 벽을 따라
    //    시트판넬 구간을 표시하는 굵은 마젠타 선이 그어진다. 마젠타는 밝기가 낮아
    //    이진화에서 '어두움'으로 잡히므로 벽으로 오인될 수 있다.
    name: '컬러 배지·마감재 오버레이가 있는 도면',
    svg: planSvg({
      w: 6600, h: 5100, ppm: 0.2,
      body(a) {
        const M = 300, W = 6000, H = 4500;
        a.wall(M, M, W, T_BEAR); a.wall(M, M + H - T_BEAR, W, T_BEAR);
        a.wall(M, M, T_BEAR, H); a.wall(M + W - T_BEAR, M, T_BEAR, H);
        a.cut(M + 5800, M + 1600, T_BEAR, DOOR);
        // 벽을 따라 그은 마감재 표시선 (실제 도면의 시트판넬 구간 표기)
        a.fill(M + 200, M + 200, 2600, 70, '#e5007f');
        a.fill(M + 200, M + H - 270, 3000, 70, '#e5007f');
        a.fill(M + 200, M + 1200, 70, 1800, '#2ea7e0');
        // 방 안에 흩어진 컬러 배지 — 벽이 아니다
        for (const [x, y, c] of [[1600, 1500, '#f29500'], [2300, 1500, '#8e6bbf'],
          [1600, 2300, '#8fc31f'], [3400, 2600, '#2ea7e0'], [4300, 1800, '#e5007f']]) {
          a.fill(M + x, M + y, 420, 420, c);
        }
        a.text(M + 2400, M + 3600, '거실');
      },
    }),
    mmPerImgPx: 1 / 0.2,
    probes: [
      { at: [3300, 2800], areaM2: [20, 26],
        note: '5,600×4,100 = 23.0㎡. 배지(420mm)나 마감재 선이 벽이 되면 방이 잘려 나간다' },
    ],
  },
  {
  ...
  {
    // 도면 인식 문헌이 쓰는 벽의 정의 — "직교하는 두 방향으로 놓인, 두께보다 긴 곧은 띠".
    // 그 기준에서 벗어나는 것들을 방 안에 잔뜩 넣고, 그래도 방이 온전히 잡히는지 본다.
    // 사용자가 "방 이름 텍스트가·기물이 벽으로 인식된다"고 지적한 것들이다.
    name: '가구·기물·글씨가 잔뜩 있는 도면',
    svg: planSvg({
      w: 6600, h: 5600, ppm: 0.22,
      body(a) {
        const M = 300, W = 6000, H = 5000;
        a.wall(M, M, W, T_BEAR); a.wall(M, M + H - T_BEAR, W, T_BEAR);
        a.wall(M, M, T_BEAR, H); a.wall(M + W - T_BEAR, M, T_BEAR, H);
        a.cut(M + 2400, M + H - T_BEAR, DOOR, T_BEAR);
        // 문 열림 호 — 굽어 있어 곧은 구간이 짧다. 굵게 그려도 벽이 되면 안 된다
        a.thin(`M${a.px(M + 2400)} ${a.px(M + H - 100)} a ${a.px(DOOR)} ${a.px(DOOR)} 0 0 1 ${a.px(DOOR)} ${a.px(-DOOR)}`, 5);
        // 뭉툭한 기물 — 변기·둥근 식탁·의자. 어느 방향으로도 길지 않다
        for (const [cx, cy, r] of [[1100, 1200, 380], [4200, 1600, 520], [4200, 3000, 240]]) {
          a.px(0);
          a.wall(M + cx - r, M + cy - r, r * 2, r * 2);      // 덩어리(원 대신 정사각형으로 단순화)
        }
        // 굵은 실명 글씨 — 방 한가운데 줄지어 있다
        [1400, 2100, 2800].forEach((x) => a.wall(M + x, M + 3600, 460, 400));
        // 해칭 무늬 — 짧은 빗금이 반복된다
        for (let x = 300; x < 1800; x += 160) a.wall(M + x, M + 4200, 60, 300);
        a.text(M + 2600, M + 2600, '거실');
      },
    }),
    mmPerImgPx: 1 / 0.22,
    probes: [
      { at: [3300, 1000], areaM2: [24.5, 28],
        note: '벽 안쪽 5,600×4,600 = 25.8㎡. 가구·글씨·해칭·문 호가 벽이 되면 방이 여러 '
            + '조각으로 갈라지거나 그만큼 파여 훨씬 작게 잡힌다' },
      { at: [3300, 4600], areaM2: [24.5, 28],
        note: '글씨 줄 아래쪽을 눌러도 같은 방 — 글자가 이어 붙어 가로막으면 여기가 따로 잡힌다' },
    ],
  },
  {
