...
(async () => {
  ...
  console.log('── 12. 복합 조건(AND) ──');
  {
     ...
     */
    /*
     * **`N평` 은 제품 규격이지 방 크기가 아니다** (2026-08-27 사용자 지적) —
     * *"19평이 넘는 모델도 일괄로 나옵니다. 원하는 건 19형이지 20형 22형 25형이 아닙니다."*
     * `면적 >= 요청` 이던 것을 **정확 매칭**으로 바꿨다. 인치와 같은 성격이다.
     * 위아래 **둘 다** 안 섞이는지 본다 — 한쪽만 보면 절반을 놓친다.
     */
    const areaOf = (p) => window.prodArea(p);
    const py = [[19, 62.6], [20, 65.9], [25, 81.8], [17, 56.9]];
    for (const [n, m2] of py) {
      const got = models(`${n}평 에어컨`);
      if (!got.some((p) => areaOf(p) === m2))
        fail(`"${n}평 에어컨"에 ${n}평형(${m2}㎡)이 없다 — 평→㎡ 환산이 자기 평형을 떨어뜨린다`);
      const off = got.map(areaOf).filter((a) => a != null && Math.abs(a - m2) > 1.0);
      if (off.length)
        fail(`"${n}평 에어컨"에 다른 평형이 섞였다 (${[...new Set(off)].join(',')}㎡) — 평형은 '이상'이 아니라 그 규격이다`);
    }
    /* **㎡ 로 친 것은 그대로 '이상' 이다** — 그쪽은 사람이 방 넓이를 말하는 것이다 */
    const byM2 = models('에어컨 62㎡');
    if (byM2.length && !byM2.some((p) => areaOf(p) > 63))
      fail('"에어컨 62㎡" 가 62㎡ 짜리만 낸다 — ㎡ 는 "이 방을 감당하는가" 라 이상이어야 한다');
    /* 평형 규격이 없는 품목(공기청정기는 청정면적)은 **물러서고 그 사실을 적는다** */
    const P4 = window.parseQuery('19평 공기청정기');
    const r4 = window.search(P4);
    if (r4.length && !(P4.notes || []).length)
      fail('평형이 정확히 안 맞아 넓혀 찾았는데 화면이 그 사실을 안 밝힌다');
    console.log(`  평형 정확 매칭 — ${py.map(([n]) => n + '평 ' + models(n + '평 에어컨').length + '종').join(' · ')} ✓`);
