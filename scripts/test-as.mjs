...
/*
 * ── 원문에 있는데 우리 화면에서 빠진 두 가지 (2026-08-30 전수 조사) ──────────
 *
 * 둘 다 **빠지면 그대로 고객 분쟁**이라 검사가 붙들고 있어야 한다.
 *
 * ① **태블릿 배터리는 1년이다.** 원문은 배터리 단서를 휴대폰·스마트폰·태블릿 **공통**으로
 *    적는데(`* 단, 배터리는 1년 적용`) 같은 앱의 스마트폰 화면만 그것을 적고 태블릿은
 *    빠져 있었다. "태블릿 배터리 2년"으로 안내하면 뒤집힌다.
 * ② **멤버십 연장 대상에 냉동고가 있다.** 원문 `일반(냉장고(300L이상), 냉동고)` 를
 *    `일반 냉장고(300L 이상)` 으로 의역해 냉동고가 사라져 있었다 — 냉장고 화면을 읽는
 *    상담사가 3년 연장 대상 고객을 대상에서 뺀다.
 */
{
  const tab = A.DB['태블릿'] || {};
  if (!/배터리[^·]*1년/.test(String(tab.baseNote || ''))) {
    fail(`태블릿에 "배터리는 1년" 이 없다 — 원문은 휴대폰·스마트폰·태블릿 공통으로 적는다 (지금: ${tab.baseNote})`);
  } else if (!/배터리[^·]*1년/.test(String((A.DB['스마트폰'] || {}).baseNote || ''))) {
    fail('스마트폰에서 "배터리는 1년" 이 사라졌다');
  } else pass('태블릿·스마트폰 모두 배터리 1년을 적는다');

  const fridge = (A.RB || {})['냉장고'];
  if (!fridge) fail('멤버십 연장 대상 표에서 냉장고 칸을 못 찾았다');
  else if (!/냉동고/.test(String(fridge[1] || ''))) {
    fail(`멤버십 연장 대상(냉장고 칸)에 냉동고가 없다 — 원문은 "일반(냉장고(300L이상), 냉동고)" 다 (지금: ${fridge[1]})`);
  } else pass('멤버십 연장 대상에 냉동고가 남아 있다');
}
 ...
 */
{
  ...
  if (!d) fail(`품목 '${KEY}' 이 없다 — 하만 계열 AS 정보가 누락된 상태다`);
    ...
    /*
     * [14-f] **케이원에이브(K1AV) 총판 제품** (2026-08-27 사장님 요청).
     *
     * 이 앱은 하만 품목에서 *"삼성전자가 국내 유통한 제품만 서비스합니다"* 라고만 적고
     * 있었다. 그런데 **K1AV 총판 제품도 서비스가 된다** — 보증서를 동봉하면. 그대로
     * 두면 상담사가 *"삼성 유통이 아니면 안 됩니다"* 로 안내하는데 **그건 틀린 말이다.**
     *
     * **출처가 다르다** — 기존 규칙은 `/wtyinfo`(보증 안내)에서 왔고 이것은
     * `/Reqsvc`(접수 지면)가 제조번호 조회 결과에 따라 띄우는 안내다. 이 품목은
     * 이 앱에서 유일하게 출처가 삼성전자서비스가 아니므로 **어느 원문인지까지** 지킨다.
     *
     * **보증서 실물은 담지 않았다** — 원문 모달이 이미지 한 장뿐이라(`jbl_guarantee.jpg`)
     * 글자로 옮길 것이 없다(설치비용 앱이 "원문이 전부 그림이라 옮기지 못했습니다"로
     * 처리한 것과 같다). 대신 접수 지면 링크를 건다.
     */
    {
      const k = (d.rules || []).find((r) => /K1AV|케이원에이브/.test(r.t));
      if (!k) {
        fail('하만 규칙에 케이원에이브(K1AV) 총판 제품 항목이 없다');
      } else if (!/보증서/.test(k.t)) {
        fail('K1AV 규칙에 "제품 보증서" 요건이 없다 — 그것이 이 규칙의 핵심이다');
      } else if (k.c !== 'need') {
        fail(`K1AV 규칙의 등급이 '${k.c}' 다 — 준비물이므로 'need' 여야 한다`);
      } else pass(`하만 규칙에 K1AV 총판 제품 + 보증서 요건 (「${k.l}」)`);

      /* 출처를 갈라 밝히는가 — 접수 지면에서 온 것을 보증 안내에서 온 것처럼 적으면 안 된다 */
      if (!d.src2 || !/harmansvc\.co\.kr\/Reqsvc/.test(d.src2[1])) {
        fail('K1AV 규칙의 출처(접수 지면)가 등재되지 않았다');
      } else {
        A.cur = KEY;
        const body = doc.getElementById('body');
        const links = [...body.querySelectorAll('a')].map((a) => a.getAttribute('href') || '');
        if (!links.some((u) => /Reqsvc/.test(u)))
          fail('화면이 접수 지면을 출처로 걸지 않는다');
        else if (!/출처가 다르다|접수/.test(body.textContent))
          fail('화면이 이 두 규칙의 출처가 다르다는 것을 밝히지 않는다');
        else pass('K1AV 규칙의 출처(접수 지면)를 화면이 따로 밝힌다');
      }

      /* 연락처 탭 택배 줄에도 있는가 — 택배로 보낼 때 동봉하는 것이라 그 자리에도 있어야 한다 */
