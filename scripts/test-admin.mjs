  ...
  }
  {
    ...
    if (at2 < 0) bad.push('lgColor 가 없다');
      ...
      if (!oh) bad.push('당사 램프의 짙은 끝을 못 읽었다 — 앵커가 낡았다');
        ...
        /* ── **당사가 더 진해야 한다** (2026-09-06 사장님 지시로 뜻이 뒤집혔다) ────────
         * 예전 규칙은 *"두 진영의 밝기가 2.2배 안쪽"* 이었다 — 둘을 대등한 짝으로
         * 보던 때의 값이다. 사장님 지시(*"LG보다 삼성이 더 부각되어야합니다"*)로
         * **일부러 비대칭**을 준다: 당사는 진하게, LG 는 옅고 흐리게.
         * 그래서 재는 것이 「벌어졌는가」에서 **「어느 쪽이 진한가」** 로 바뀐다.
         *
 ...
 * *"빠지지 않게 검출하는 방법을 만들어 주세요"* · *"2025년 자료까지 남기게"*. */
{
  ...
  if (fs.existsSync(ixp) && fs.existsSync(rvp)) {
    ...
    /* **2026-09-05 사장님 지시로 2023-01-01** — *"[수집기준]은 23년도이후자료면 충분함"*.
       그 앞은 담아도 뜻이 없다(브랜드 개명으로 당사 옛 글만 구조적으로 안 걸린다). */
    if (!rv.includes("var MIN_YMD = '2023-01-01';")) bad.push('수집 기준이 2023-01-01 이 아니다');
 ...
 * 때문이다. 셋 다 여기서 지킨다. */
{
  ...
  if (at >= 0) {
    ...
    const blk = ix.slice(Math.max(0, at - 4000), at + 300);
 ...
 * 옛 남색↔벽돌빨강은 ③이 41 이었고, 그것을 되돌리면 이 검사가 문다. */
{
  ...
  if (!sLo || !sHi || !lLo || !lHi) bad.push('히트맵 램프를 못 읽었다 — 앵커가 낡았다');
    ...
    /* ── **두 진영이 갈리는 길이 둘이다** (2026-09-06 사장님 지시로 넓혔다) ─────────
     * *"LG보다 삼성이 더 부각되어야합니다"* 를 지키려면 LG 채도를 내려야 하는데,
     * **채도를 내리면 색상 거리도 함께 줄어든다**(선명한 코랄 60 → 흐린 토프 50).
     * 그렇다고 못 가르는 것이 아니다 — 그때는 **가르는 단서가 색상이 아니라 채도**다:
     * 진한 청록 옆의 흐린 회갈색은 색을 못 가려도 「진한 쪽 / 흐린 쪽」으로 갈린다.
     * 그래서 **둘 중 하나만 넘기면 된다** — 색상 거리 55 이상 **또는** 채도 차 40 이상.
     * 옛 남색↔벽돌빨강은 거리 41 · 채도 차 15 라 **둘 다 못 넘긴다.** */
    let sep = 999;
    for (let i = 1; i <= 5; i++) {
    ...
    const sat = (c) => {
      const M = Math.max(c[0], c[1], c[2]) / 255, m = Math.min(c[0], c[1], c[2]) / 255, L = (M + m) / 2;
      return M === m ? 0 : Math.round((L > 0.5 ? (M - m) / (2 - M - m) : (M - m) / (M + m)) * 100);
    };
    const gap = sat(sHi) - sat(lHi);
    /* **당사가 확실히 더 선명해야 한다** — 이것이 「부각」의 실체이고, 동시에
    ...
    /* **색상도 갈려야 한다** — 채도만 다르고 색이 같으면(진한 청록 ↔ 옅은 청록) 가운데
       ...
       **적록 색각 거리로는 이걸 못 잡는다** — 밝기 차가 커서 거리가 크게 나온다.
       그래서 색상각으로 본다(지금 청록 186° ↔ 토프 12° = 174° 벌어졌다). */
    const hue = (c) => {
    ...
    const dh = Math.abs(hue(sHi) - hue(lHi));
 ...
 *
 * **2026-09-07 사장님 재지시로 그 줄을 통째로 뺐다** — *"삼성숫자 엘지숫자 이게 더
 * 직관적입니다. 히트맵안에 점명이 써있기때문입니다"*. 칸에 우리 점명이 이미 크게 있고
 * 짝은 1:1 이라, 어느 LG 지점인지는 말풍선에서 확인하면 되는 부가 정보다.
 * 두 라벨이 없는 좁은 칸에서는 남긴다 — 거기서는 그 줄이 유일한 LG 정보다. */
{
  const ix = fs.readFileSync(new URL('../docs/apps-script/ReviewsIndex.html', import.meta.url), 'utf8');
  const bad = [];
