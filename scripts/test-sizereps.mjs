...
const GOLDEN = [
  ...
  /*
   * **축 순서가 다른 표기**를 고정한다. 오드앵글 선풍기는 카탈로그가 `370 × 390 × 840(1090)`
   * 로 적는데, 괄호는 **높이 조절 범위**라 그 자리가 높이다(`W × D × H`).
   * 라벨만 보고 W×H×D 로 읽으면 발자국이 370×390 대신 **370×840** 이 되어 두 배 넘게
   * 잡히고 "들어갑니다" 판정이 그대로 거짓이 된다. 축이 되돌아가면 여기서 걸린다.
   */
  ['리빙 선풍기', '폭 370~400mm', 'OD-F3565DW', '본체', [370, 840, 390]],
];
let goldBad = 0;
/*
