 ...
 */
{
  const KNOWN = new Map([
    ...
    /*
     * 식기세척기 두 코드는 **2026-08-16 에 실재가 확인돼** 여기서 뺐다.
     * 옛 판단("현행 11종에 없다")은 **목록 지면만 본 것**이었는데, 지원 페이지는
     * 목록에 없는 모델도 해결해 준다 —
     *   samsung.com/sec/support/model/DW80F71Y1UEWS/ → "Bespoke AI 식기세척기 빌트인
     *   12인용 DW80F71Y1UEW" (HTTP 200)
     *   .../DW80F73Y1UEWS/ → "빌트인 14인용 DW80F73Y1UEW" (HTTP 200)
     * 반대로 카탈로그가 적은 `DW80F73Y1UWW` 는 **HTTP 500 에 제품명도 없다** — 그쪽이
     * 틀린 코드였다. `DW80F73X1UEW`(X 변형)도 실재하므로 Y·X 는 함께 두는 것이 맞다
     * (뒷자리만 다른 코드는 색상 변형일 수 있다 — `U01`/`USW` 와 같은 규칙).
     * **모델코드를 "없다"고 판정할 때 목록 지면만 보지 말 것.**
     *
     * 실재가 확인됐으므로 이제 성격이 위쪽 항목들과 같다 — **실제 제품인데 파인더에
     * 아직 없다.** 다만 파인더는 카탈로그가 기준이라(CLAUDE.md) 지원 페이지만 보고
     * 스펙 없이 제품을 넣지는 않는다. 다음 카탈로그 대조 때 함께 들어가야 한다.
     */
    ['DW80F71Y1UEWS', '파인더 미수록 (Bespoke AI 식기세척기 빌트인 12인용 — 2026-08-16 지원 페이지로 실재 확인, 카탈로그 미수록)'],
    ['DW80F73Y1UEWS', '파인더 미수록 (Bespoke AI 식기세척기 빌트인 14인용 — 2026-08-16 지원 페이지로 실재 확인. 카탈로그의 DW80F73Y1UWW 는 500 이라 그쪽이 틀린 코드였다)'],
  ]);
